...
function CallCard(props) {
  ...
  function archiveFunc(event) {
    ...
    console.log(data)
    props.Archived.insert(event.target.id, data)
    setArrowType(1)
  }
  function unarchiveFunc(event) {
    let { data } = props.Archived.remove(event.target.id);
    console.log(data)
    props.unArchived.insert(event.target.id, data)
    setArrowType(1)
  }
  ...
  function contact_list() {
    ...
    if (call_list1.length === 0) {
      props.archive_type === 1 ? result.push(<div style={{ textAlign: 'center', marginTop: '50%', fontSize: '400%', fontWeight: '700', color: 'blue' }}><IoMdArchive /></div>) : result.push(<div style={{ textAlign: 'center', marginTop: '50%', fontSize: '200%', fontWeight: '700', color: 'red' }}>No logs </div>)
      return result
